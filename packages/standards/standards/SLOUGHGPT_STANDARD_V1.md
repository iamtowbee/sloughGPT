...
5. **CLI**: `python scripts/validate_dataset_manifest.py <path-to-manifest.json> [--resolve]` (runtime resolution). **Schema CI**: `python scripts/validate_standards_schemas.py` (`jsonschema` via `pip install -e ".[dev]"` or `pip install jsonschema`; example JSON under `standards/v1/examples/`).
