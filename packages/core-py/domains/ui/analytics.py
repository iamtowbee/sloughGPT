...
class AnalyticsManager:
    ...
    def update_training_status(
        self, status: str, epoch: int = 0, loss: float = 0.0, lr: float = 0.0003
    ) -> None:
        """Update training status."""
        with self._lock:
            self.epochs_completed = epoch
            self.performance_data.append(
                {"epoch": epoch, "loss": loss, "learning_rate": lr, "timestamp": time.time()}
            )

    def log_training_event(self, event: str, level: str = "info") -> None:
        """Log a training event."""
        self.training_logs.append({"event": event, "level": level, "timestamp": time.time()})
