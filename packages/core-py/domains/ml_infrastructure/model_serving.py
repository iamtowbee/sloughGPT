...
class ModelServer:
    ...
    def register_endpoint(
        ...
        self.batching[endpoint.name] = BatchingManager(
            max_batch_size=endpoint.max_batch_size, timeout_ms=endpoint.batch_timeout_ms
        )

        logger.info(f"Registered endpoint: {endpoint.name}")

    def predict(
        self, endpoint_name: str, inputs: Any, parameters: Optional[Dict[str, Any]] = None
    ) -> InferenceResponse:
...
class ModelRouter:
    ...
    def add_route(self, endpoint: str, model_version: str, weight: float, model_fn: Callable):
        ...
        self.routes[endpoint].append(
            {"version": model_version, "weight": weight, "model_fn": model_fn}
        )
