...
from open_webui.routers import openai, ollama, cerebro
...
async def fetch_cerebro_models(request: Request, user: UserModel = None):
    """Fetch Cerebro models"""
    try:
        cerebro_response = await cerebro.get_cerebro_models(request, user=user)
        return cerebro_response["data"]
    except Exception as e:
        log.error(f"Failed to fetch Cerebro models: {e}")
        return []
