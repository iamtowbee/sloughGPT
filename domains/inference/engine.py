...
class InferenceEngine:
    ...
    def _apply_repetition_penalty(
        ...
        for token_id in prev_tokens.unique():
            ...
            logits[token_id] = torch.where(mask, logits[token_id] * penalty, logits[token_id] / penalty)
    ...
    def generate_single(
        ...
        with torch.no_grad():
            for _ in range(max_new_tokens):
                ...
                prev_tokens = torch.cat([prev_tokens, torch.tensor([[next_token]], device=self.device)], dim=1)
        ...
        with torch.no_grad():
            for _ in range(max_new_tokens):
                ...
                next_token = await loop.run_in_executor(
                    None,
                    lambda: self._sample_token(logits, temperature, top_k, top_p)
                )
