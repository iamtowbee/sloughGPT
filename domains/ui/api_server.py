...
from domains.ai_personality import PersonalityManager, PersonalityType, list_personalities, get_personality_manager
...
async def start_training(
    ...
    def train_background():
        ...
        try:
            ...
            # Apply LoRA
            if use_lora:
                lora_config = LoRAConfig(rank=lora_rank, alpha=16, target_modules=['c_attn', 'c_proj', 'c_fc'])
...
async def quantize_model(
    ...
    torch.save({
        'model': quantized_model.state_dict() if hasattr(quantized_model, 'state_dict') else checkpoint['model'],
...
async def get_logs(limit: int = 100):
    ...
    return {
        "logs": _request_logs[-limit:],
        "count": len(_request_logs)
    }
...
async def load_model(model_name: str):
    ...
    try:
        if model_name == "sloughgpt":
            ...
            model_id = f"gpt2" if model_name == "gpt2" else f"gpt2-{model_name.replace('gpt2-', '')}"
