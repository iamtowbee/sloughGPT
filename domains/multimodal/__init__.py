...
class VisionEncoder(nn.Module):
    ...
    def __init__(self, config: MultiModalConfig):
        ...
        self.position_embed = nn.Parameter(
            torch.zeros(1, (config.image_size // config.patch_size) ** 2 + 1, config.vision_hidden_size)
        )
...
class CLIPModel(nn.Module):
    ...
    def encode_text(self, text_tokens: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Encode text."""
