...
def create_dataset(name: str, text: str = None, file_path: str = None, folder_path: str = None):
    ...
    if text:
        ...
        if result.returncode == 0:
            ...
            return {"success": True, "message": f"Dataset '{name}' created from folder"}
        else:
        ...
        return {"success": True, "message": f"Empty dataset template '{name}' created"}
    ...
    if not folder_path:
        ...
        if prepare_src.exists():
            ...
            if result.returncode == 0:
                ...
                return {"success": True, "message": f"Dataset '{name}' ready for training"}
            else:
                print(f"❌ Preparation failed: {result.stderr}")
                return {"success": False, "error": result.stderr}
        else:
            print(f"❌ prepare.py not found at {prepare_src}")
            return {"success": False, "error": f"prepare.py not found at {prepare_src}"}
