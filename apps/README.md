...
1. **`./verify.sh`** — checks core paths; runs the same ruff smoke as CI when `ruff` is installed (use `pip install -e ".[dev]"` if needed). Also prints **`ci_cd.yml`** parity commands (**`test-web`**, **`test-sdk-ts`**, **`sdk-test-py`**, **`standards-schemas`**).
